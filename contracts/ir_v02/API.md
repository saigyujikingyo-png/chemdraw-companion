...
The bond donor budget uses both actual bond-order decrease and available sigma/pi electrons; multiple pi pairs from a triple bond remain possible. Fractional aromatic and nonlocal electron-transfer profiles are explicitly outside complete accounting coverage. A bond sink must share a chemical atom with its source; atom sinks may be nonendpoints of a source bond when a related increasing bond connects them.
