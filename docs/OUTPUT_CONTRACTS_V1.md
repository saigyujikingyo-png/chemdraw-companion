...
## Pinned Origin reference for the next implementation

Read-only reference: Origin Companion 0.2.11 at [`8bff772b6cb70019e19a13e97adef9568b569d20`](https://github.com/saigyujikingyo-png/origin-agent-bridge/commit/8bff772b6cb70019e19a13e97adef9568b569d20), including its [output-contract documentation](https://github.com/saigyujikingyo-png/origin-agent-bridge/blob/8bff772b6cb70019e19a13e97adef9568b569d20/origin-agent/docs/OUTPUT_CONTRACTS.md). This is source review, not an executed Origin test or transferred acceptance result. The existing 20-row ChemDraw registry remains the coverage and ownership ledger; no implementation/server/host status is upgraded.

| Reference pattern and reviewed source | ChemDraw application and remaining evidence |
| --- | --- |
| Strict shared types, forbidden extras/non-finite numbers, no serialization that fills defaults ([types lines 25–37](https://github.com/saigyujikingyo-png/origin-agent-bridge/blob/8bff772b6cb70019e19a13e97adef9568b569d20/origin-agent/src/origin_agent/output_types.py#L25), [validation lines 275–305](https://github.com/saigyujikingyo-png/origin-agent-bridge/blob/8bff772b6cb70019e19a13e97adef9568b569d20/origin-agent/src/origin_agent/output_contracts.py#L275)) | Implement a common producer boundary for all five CLI operations and later MCP adapters; preserve omitted/null/unknown and original measurements. Prove rejection of malformed real producer results separately from the existing synthetic schema checks. |
| Bad-output identity retention and recovery guidance ([lines 207–229](https://github.com/saigyujikingyo-png/origin-agent-bridge/blob/8bff772b6cb70019e19a13e97adef9568b569d20/origin-agent/src/origin_agent/output_contracts.py#L207)) | Origin extracts valid IDs from a still-parseable payload. ChemDraw must additionally retain trusted pre-dispatch request/operation/session/document identity and revision when payload parsing fails. Test post-effect malformed output without a repeated write; do not report `native_write=false` merely because validation failed. |
| Per-operation type map, on-demand schemas, compact projection and schema caching ([type map](https://github.com/saigyujikingyo-png/origin-agent-bridge/blob/8bff772b6cb70019e19a13e97adef9568b569d20/origin-agent/src/origin_agent/output_types.py#L741), [schema construction lines 66–172](https://github.com/saigyujikingyo-png/origin-agent-bridge/blob/8bff772b6cb70019e19a13e97adef9568b569d20/origin-agent/src/origin_agent/output_contracts.py#L66)) | Reuse the existing CLI action switch and planned help discovery; validate the selected operation's full contract before any compact outward projection. The reviewed files do not establish server registration or dispatch wiring: actual direct/dispatch calls and host schema resolution remain required. |
| Artifact metadata/content-block and text-pagination consistency ([lines 232–272](https://github.com/saigyujikingyo-png/origin-agent-bridge/blob/8bff772b6cb70019e19a13e97adef9568b569d20/origin-agent/src/origin_agent/output_contracts.py#L232)) | Bind native PNG and any derived preview to distinct provenance and the actual returned block. Validate kind, MIME, resource reference and pagination; verify file bytes/hash independently. The reviewed validator does not decode binary content to recompute its hash and does not prove host receipt. Do not copy its fixed block index. |
| Bounded JSON and consistent text fallback ([lines 175–204](https://github.com/saigyujikingyo-png/origin-agent-bridge/blob/8bff772b6cb70019e19a13e97adef9568b569d20/origin-agent/src/origin_agent/output_contracts.py#L175), [result assembly](https://github.com/saigyujikingyo-png/origin-agent-bridge/blob/8bff772b6cb70019e19a13e97adef9568b569d20/origin-agent/src/origin_agent/output_contracts.py#L283)) | Keep full XML/object diffs in artifacts and return bounded summaries/references; enforce finite JSON, depth/count/byte bounds and equivalent structured/text results without duplicating media. Determine ChemDraw-specific limits and measure discovery size and actual usage separately. |

Keep ChemDraw's existing envelopes, operation semantics, owned-process/document binding and revision checks. Do not import Origin's job/session enums, GUI operations, full/economy tool inventory, numeric budgets or dependency choice without a ChemDraw-specific need and review. Machine Interface Only remains mandatory. This reference adds planned producer-failure, dispatch and media checks to the existing acceptance gates; it neither unfreezes runtime nor authorizes another native run.
