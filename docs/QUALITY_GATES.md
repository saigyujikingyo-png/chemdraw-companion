...
| M2 / examples/m2-beckmann-snake.json | Unsymmetrical phenyl-anti oxime, seven states, six transitions, fifteen curly arrows, explicit proton transfers, lone pairs/charges and two snake turns | Native CDX AND CDXML separately reopened/edited; stable IR mappings, full-scene render, untouched first output, correction timing and controlled failure attribution |
...
2. **Chemical semantics:** compare graph identity, bond orders, isotopes, formal charge, explicit H and specified stereochemistry before/after native round trip, independently where possible. For reactions verify participant roles and mapping, total charge and complete balance or declared omissions. For M1 verify two flows; for M2 verify every state, fifteen flows, anti migration and declared graph changes. SMILES equality cannot check arrows or labels.
...
M2 additionally rejects wrong migrating substituent, missing proton/water mapping, nonexistent lone-pair donors, broken snake traversal, confused step/electron arrows, ambiguous repeated-state object matching and human edits counted as automatic success. Use the controls in M2_BECKMANN_SNAKE.md for attribution.
...
## Blocking report and architecture diagnosis

Stop peripheral expansion when transport, object control or composition fails, but continue bounded diagnosis and independent core work. Return build/edition/backend, failed operation, sanitized vendor error, reproducer, last safe artifact and controlled alternatives. Missing native access is a route-specific blocker; a valid native save is not composition evidence.

For M2 use manual native reference, frozen geometry replay and Composer output to distinguish native control, adapter roundtrip, IR/semantics, Composer geometry, mixed or undetermined causes. Record first-output quality and actual correction time. Do not pivot to another product or add packaging to avoid this diagnosis. Mnova remains a separately authorized future product decision, not a ChemDraw fallback.
