...
Under the latest Quality Oracle phase policy, M1 is permitted exposed development data; M2 and descendants remain locked against tuning/training or correction-prior use. See GOLD_SILVER_HOLDOUT_V1.md. Independent development examples drive general rules. After the base gates and an immutable implementation freeze, an independent evaluator selects the previously unused asymmetric-oxime holdout. Continue to keep MCP, installer, extra hosts and release expansion behind the core gates.
