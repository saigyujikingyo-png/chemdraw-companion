...
def run(payload,geometry,out):
    ...
    records.append({'case':'bijective_ids_and_shuffled_arrays','max_mapped_atom_displacement_pt':maximum,'observed_layout_identity':'pass' if maximum<.02 else 'failed','counts':validate_semantics(nm),'clearance_failures':len(changed['layout_diagnostics']['curve_clearance_failures'])})
    ...
    try:
        ...
        records.append({'case':'intrinsic_component_rotation_37_degrees','semantics_and_fit':'pass','stereo_checks':changed['stereo_checks'],'clearance_failures':len(changed['layout_diagnostics']['curve_clearance_failures']),'quality_acceptance':'not_implied'})
    ...
    report={'scope':'metamorphic and structural controls only; no unseen chemical selection','baseline_clearance_failures':len(baseline['layout_diagnostics']['curve_clearance_failures']),'cases':records,'metamorphic_behavior':'partial; native geometry regeneration and quality remain separate','holdout':'not_started'}
...
if __name__=='__main__':
    ...
    print(json.dumps(run(read(a.request),g,a.out)))
