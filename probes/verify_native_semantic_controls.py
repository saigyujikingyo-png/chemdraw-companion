...
from runtime.native_semantics import (observe_document,compare_document,PROFILE,control_expectation,
    derive_branch_coverage,semantic_signature)
...
def main():
    ...
    for control in matrix['controls']:
        ...
        for phase,folder,sidecar_field,binding_field in [('cleanup',args.native/'clean','cleanup_atom_readback','cleanup_process'),
            ...
            observed=observe_document(path,sidecar,source_freeze=args.source_freeze,qualification_control=True)
            comparison=compare_document(observed,expected,control['expected_bonds'])
            adapter={}
            try:
                measured=read_explicit_geometry(path,expected,control['expected_bonds'],atom_readback=sidecar,semantic_source_freeze=args.source_freeze,semantic_qualification_control=True)
        ...
        consistent=semantic_signature(phases['cleanup']['observations'])==semantic_signature(phases['reopen']['observations'])
    ...
    coverage=derive_branch_coverage(all_records)
    admitted_branches=[r['branch'] for r in coverage]
    for control,record in zip(matrix['controls'],all_records):
        expected=control_expectation(control)
        for phase,folder in [('cleanup',args.native/'clean'),('reopen',args.native)]:
            path=folder/control['file'];sidecar=read_json(path.with_suffix('.atoms.json'))
            try:
                measured=read_explicit_geometry(path,expected,control['expected_bonds'],atom_readback=sidecar,
                    semantic_source_freeze=args.source_freeze,semantic_admitted_branches=admitted_branches)
                adapter=dict(status='match',mode='measured_branches_only',atom_count=len(measured['atoms']),bond_count=len(measured['bonds']))
            except ValueError as exc:
                adapter=dict(status='refused',mode='measured_branches_only',diagnostic=exc.as_dict() if hasattr(exc,'as_dict') else dict(message=str(exc)))
            if adapter['status']!=record['phases'][phase]['production_adapter']['status']:
                qualified=False
            record['phases'][phase]['production_adapter']=adapter
        (args.out/(Path(control['file']).stem+'.semantic.json')).write_bytes((json.dumps(record,indent=2)+'\n').encode())
    ...
    evidence_files=[]
    for directory in (args.native,args.out):
        for p in sorted(directory.rglob('*')):
            if p.is_file() and p.name!='events.jsonl':
                evidence_files.append(dict(file=directory.name+'/'+p.relative_to(directory).as_posix(),bytes=p.stat().st_size,sha256=digest(p)))
    report=dict(version='native-semantic-control-receipt/1.1',profile=PROFILE,source_freeze_sha256=frozen['sha256'],
        ...
        qualification_status='qualified_covered_branches' if qualified else 'partial_not_qualified',
        branch_coverage=coverage,admitted_branches=admitted_branches,native_folder=args.native.name,analysis_folder=args.out.name,
        evidence_files=evidence_files,qualification_scope='Only the listed actually observed branches, not the entire proposed profile.',
